        ...
        stage('Deploy') {
            steps {
                sshagent(credentials: ['target-ec2-deploy-key']) {
                    sh '''
                        scp -o StrictHostKeyChecking=no -r ./* ec2-user@<13.206.198.121>:/tmp/website/
                        ssh -o StrictHostKeyChecking=no ec2-user@<13.206.198.121> "sudo rm -rf /usr/share/nginx/html/* && sudo cp -r /tmp/website/* /usr/share/nginx/html/"
                    '''
                }
            }
        }
